...
impl TermCaps {
  ...
  pub fn detect() -> Self {
    ...
    let is_windows_cmd = is_windows && !is_windows_terminal && term.is_empty() && term_program.is_empty();
    ...
    let unicode = !is_windows_cmd || is_windows_terminal || is_conemu || lang.to_lowercase().contains("utf");
...
mod tests {
  ...
  fn detect_returns_sensible_defaults() {
    ...
    // Verify the struct fields are accessible (type system ensures valid values)
    let _ = caps.unicode;
    let _ = caps.true_color;
    let _ = caps.osc8_links;
  }
